        ...
        match component {
            Component::Normal(name) => {
                ...
                if name_str.starts_with('.') {
                    return Err(ValidationError::InvalidPathComponent(format!(
...
pub fn validate_path_within_root(path: &Path, root: &Path) -> Result<(), ValidationError> {
    ...
    let canonical_path = path.canonicalize().map_err(|_| {
        ValidationError::InvalidPathComponent("Cannot canonicalize path".to_string())
    })?;
    let canonical_root = root.canonicalize().map_err(|_| {
        ValidationError::InvalidPathComponent("Cannot canonicalize root".to_string())
    })?;
