...
impl StorageUtilities {
    ...
    pub async fn generate_report(&self) -> Result<StorageReport> {
        ...
        let recommendations = self.generate_recommendations(&health, &filesystem, &corruption, &performance);
        ...
        if root_path.exists() {
            ...
            while let Some(entry) = dir.next_entry().await? {
                ...
                if path.is_dir() {
                    ...
                    } else if shard_name.len() == 2 && shard_name.chars().all(|c| c.is_ascii_hexdigit()) {
    ...
    fn generate_recommendations(
        ...
        if filesystem.orphaned_files > 0 {
            recommendations.push(format!("Clean up {} orphaned files", filesystem.orphaned_files));
        }
        
        if filesystem.temp_files > 100 {
            recommendations.push("High number of temp files - run cleanup_expired_partials()".to_string());
        }
        ...
        if !filesystem.shard_distribution.is_empty() {
            ...
            if max > min * 3 {
                recommendations.push("Shard distribution is unbalanced - consider rehashing".to_string());
            }
        }
        ...
        if !corruption.corrupt_objects.is_empty() {
            recommendations.push(format!("Remove {} corrupt objects", corruption.corrupt_objects.len()));
        }
        ...
        if performance.throughput_mbps < 50.0 {
            recommendations.push("Low throughput - check disk performance and consider SSD".to_string());
        }
    ...
    pub async fn perform_maintenance(&self, options: &MaintenanceOptions) -> Result<MaintenanceReport> {
    ...
    async fn analyze_sub_shard(&self, sub_shard_path: &Path) -> Result<(u64, u64, Vec<ObjectInfo>)> {
        let mut object_count = 0u64;
        let mut total_size = 0u64;
        let mut objects = Vec::new();
    ...
    async fn verify_shard_integrity(&self, shard_path: &Path) -> Result<(u64, u64, Vec<CorruptObject>, Vec<HashMismatch>)> {
        let mut checked = 0u64;
    ...
    async fn verify_sub_shard_integrity(&self, sub_shard_path: &Path) -> Result<(u64, u64, Vec<CorruptObject>, Vec<HashMismatch>)> {
        let mut checked = 0u64;
        let mut corrupt = 0u64;
        let mut corrupt_objects = Vec::new();
        let mut hash_mismatches = Vec::new();
