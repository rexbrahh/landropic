...
async fn test_streaming_vs_legacy_write() {
    ...
    let streaming_throughput =
        (data.len() as f64) / streaming_duration.as_secs_f64() / (1024.0 * 1024.0);
...
async fn test_streaming_read_performance() {
    ...
    assert!(
        throughput > 80.0,
        "Read throughput too low: {:.2} MB/s",
        throughput
    );
...
async fn test_batch_performance() {
    ...
    let chunk_data: Vec<Vec<u8>> = (0..num_chunks)
        .map(|i| {
    ...
    let chunks: Vec<(&[u8], Option<ContentHash>)> = chunk_data
        .iter()
        .map(|data| (data.as_slice(), None))
        .collect();
    ...
    println!(
        "Batch write: {} chunks in {:?} = {:.2} chunks/sec, {:.2} MB/s",
    ...
    assert!(
        chunks_per_sec > 200.0,
        "Batch throughput too low: {:.2} chunks/sec",
        chunks_per_sec
    );
}
...
async fn test_batch_streaming_performance() {
    ...
    println!(
        "Streaming batch write: {} chunks in {:?} = {:.2} chunks/sec, {:.2} MB/s",
        num_chunks, duration, chunks_per_sec, throughput
    );
    ...
    assert!(
        ...
        "Streaming batch throughput too low: {:.2} chunks/sec",
        chunks_per_sec
    );
}
...
async fn test_large_file_streaming() {
    ...
    println!(
        "Large file (100MB) - Write: {:.2} MB/s, Read: {:.2} MB/s",
