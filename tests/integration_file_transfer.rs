...
use landro_daemon::{
    bloom_sync_integration::BloomSyncEngine, discovery::DiscoveryService,
    sync_engine::create_enhanced_sync_engine, watcher::FileWatcher, FileTransferClient,
    OrchestratorConfig, SyncOrchestrator,
};
...
async fn setup_test_daemon(
    temp_dir: &TempDir,
) -> Result<(Arc<QuicServer>, std::net::SocketAddr), Box<dyn std::error::Error + Send + Sync>> {
...
async fn test_end_to_end_file_transfer() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    match transfer_result {
        Ok(Ok(())) => {
            tracing::info!(
                "File transfer completed in {:.2} seconds",
                duration.as_secs_f64()
            );
            assert!(
                duration < Duration::from_secs(5),
                "Transfer took too long: {:.2}s",
                duration.as_secs_f64()
            );
        }
        Ok(Err(e)) => {
...
async fn test_large_file_transfer() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    match transfer_result {
        Ok(Ok(())) => {
            tracing::info!(
                "Large file transfer completed in {:.2} seconds",
                duration.as_secs_f64()
            );
            // For 1MB file, should be reasonably fast
            assert!(
                duration < Duration::from_secs(10),
                "Large file transfer took too long: {:.2}s",
                duration.as_secs_f64()
            );
        }
        Ok(Err(e)) => {
