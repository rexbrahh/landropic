...
async fn test_storage_pipeline() {
    ...
    // Create test node
    let node = CoreTestNode::new("test-storage")
        .await
    ...
    // Create test file
    let file_path = node
        .create_file("pipeline_test.txt", test_content)
        .await
    ...
    // Verify file exists
    assert!(
        node.has_file("pipeline_test.txt").await,
        "File should exist"
    );
    ...
    for chunk in &chunks {
        ...
        let read_data = node
            ...
            .read(&chunk.hash)
            .await
            .expect("Failed to read from CAS");
...
async fn test_quic_connection() {
    ...
    // Create two nodes
    let node1 = CoreTestNode::new("quic-server")
        .await
        .expect("Failed to create server node");
    let node2 = CoreTestNode::new("quic-client")
        .await
    ...
    let client_connection =
        tokio::time::timeout(Duration::from_secs(5), client.connect_addr(server_addr))
            .await
...
async fn test_data_integrity() {
    ...
    let node = CoreTestNode::new("integrity-test")
        .await
        .expect("Failed to create node");
    ...
    for (name, expected_content) in &test_cases {
        ...
        if !expected_content.is_empty() {
            ...
            assert_eq!(
                reconstructed, *expected_content,
                "Reconstructed content should match for {}",
                name
            );
        }
    }
...
async fn test_performance_basic() {
    ...
    let node = CoreTestNode::new("perf-test")
        .await
        .expect("Failed to create node");
    ...
    node.create_file("performance_test.bin", &test_content)
        .await
    ...
    let read_content = node
        .read_file("performance_test.bin")
        .await
        .expect("Failed to read file");
    ...
    info!(
        "Chunking {} bytes into {} chunks took: {:?}",
        file_size,
        chunks.len(),
        chunk_time
    );
    ...
    assert!(
        ...
        "Creation should be under 5 seconds"
    );
    assert!(
        read_time.as_millis() < 1000,
        "Read should be under 1 second"
    );
    assert!(
        chunk_time.as_millis() < 2000,
        "Chunking should be under 2 seconds"
    );
    assert!(!chunks.is_empty(), "Should produce chunks");

    // Calculate throughput
    let throughput_mbps =
        (file_size as f64 / (1024.0 * 1024.0)) / creation_time.as_secs_f64() * 8.0;
...
async fn test_concurrent_operations() {
    ...
    let node = CoreTestNode::new("concurrent-test")
        .await
        .expect("Failed to create node");
    ...
    for i in 0..5 {
        // Reduced from 10 to 5 for simpler testing
    ...
    info!(
        "Successfully created {} files concurrently",
    ...
    for filename in &successful_files {
        ...
        let expected_content = format!(
            "Concurrent file content number {}",
