...
impl TestDaemon {
    ...
    pub async fn start(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        info!(
            "Starting test daemon {} on {}",
            self.device_name, self.bind_addr
        );
        ...
        let mut server = QuicServer::new(self.identity.clone(), self.verifier.clone(), quic_config);
        ...
        let client =
            QuicClient::new(self.identity.clone(), self.verifier.clone(), client_config).await?;
    ...
    pub async fn connect_to(
        &self,
        other: &TestDaemon,
    ) -> Result<Arc<landro_quic::Connection>, Box<dyn std::error::Error>> {
        ...
        // Perform handshake
        connection
            .client_handshake(&self.device_id, &self.device_name)
            .await?;
...
async fn test_basic_file_sync() {
    ...
    assert!(
        node1.has_file("test.txt").await,
        "File should exist on node1"
    );
    let read_content = node1
        .read_file("test.txt")
        .await
    ...
    assert!(
        stats1.total_file_size > 0,
        "Total file size should be positive"
    );
...
async fn test_discovery_and_connection() {
    ...
    assert!(
        connection.remote_address().port() == 7804,
        "Should connect to correct port"
    );
    ...
    assert_eq!(
        remote_name,
        Some("test-device-7804".to_string()),
        "Should get correct device name"
    );

    // Test bidirectional connection
    let reverse_connection = node2
        .connect_to(&node1)
        .await
        .expect("Failed to establish reverse connection");
    let reverse_remote_name = reverse_connection.remote_device_name().await;
    assert_eq!(
        reverse_remote_name,
        Some("test-device-7803".to_string()),
        "Should get correct reverse device name"
    );
...
async fn test_storage_deduplication() {
    ...
    let test_content =
        b"Identical content for deduplication testing. This should be deduplicated across nodes.";

    node1
        .create_file("identical1.txt", test_content)
        .await
        .expect("Failed to create file on node1");
    node2
        .create_file("identical2.txt", test_content)
        .await
        .expect("Failed to create file on node2");

    // Create duplicate within same node
    node1
        .create_file("duplicate1.txt", test_content)
        .await
        .expect("Failed to create duplicate on node1");

    // Get storage statistics
    let stats1 = node1
        .get_storage_stats()
        .await
        .expect("Failed to get node1 stats");
    let stats2 = node2
        .get_storage_stats()
        .await
        .expect("Failed to get node2 stats");
...
async fn test_performance_benchmark() {
    ...
    node1
        .create_file("large_file.bin", &test_content)
        .await
    ...
    assert!(
        benchmark.throughput_mbps > 10.0,
        "Throughput should be reasonable for local operations"
    );
    assert!(
        benchmark.latency_ms < 1000.0,
        "Connection latency should be under 1 second"
    );
    assert_eq!(
        stats1.total_file_size, file_size as u64,
...
async fn test_error_recovery() {
    ...
    node1
        .create_file("recovery_test.txt", test_content)
        .await
        .expect("Failed to create test file");
    ...
    // Restart node2
    let mut new_node2 = TestDaemon::new(7810)
        .await
        .expect("Failed to recreate node2");
    new_node2.start().await.expect("Failed to restart node2");

    // Test recovery by re-establishing connection
    let recovery_start = Instant::now();
    let recovered_connection = node1
        .connect_to(&new_node2)
        .await
        .expect("Failed to recover connection");
    ...
    assert_eq!(
        remote_name,
        Some("test-device-7810".to_string()),
        "Should recover device information"
    );

    // Verify data integrity after restart
    assert!(
        node1.has_file("recovery_test.txt").await,
        "Original file should still exist"
    );
    let recovered_content = node1
        .read_file("recovery_test.txt")
        .await
        .expect("Failed to read recovered file");
    assert_eq!(
        recovered_content, test_content,
        "File content should be intact after recovery"
    );
...
async fn test_concurrent_operations() {
    ...
    for i in 0..10 {
        assert!(
            node.has_file(&format!("concurrent_{}.txt", i)).await,
...
async fn test_data_integrity() {
    ...
    for (name, expected_content) in &test_cases {
        ...
        assert_eq!(
            ...
            "File content should match after storage"
        );
    ...
    for (name, content) in &test_cases {
        if content.len() > 0 {
            ...
            assert_eq!(
                ...
                "Reconstructed content should match original for {}",
                name
            );
        }
    }
