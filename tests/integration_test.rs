...
async fn test_quic_handshake() {
    ...
    let client_config = QuicConfig::new().idle_timeout(Duration::from_secs(5));

    // Start server
    let mut server = QuicServer::new(server_identity.clone(), server_verifier, server_config);
    ...
    let client = QuicClient::new(client_identity.clone(), client_verifier, client_config)
        .await
        .expect("Failed to create client");

    println!("Connecting to server at {}", server_addr);

    let client_connection = timeout(Duration::from_secs(5), client.connect_addr(server_addr))
        .await
...
async fn test_storage_stack() {
    ...
    let db = DatabasePool::new(&index_path).unwrap();
    ...
    let file_id = db.upsert_file(file_entry).await.unwrap();
...
async fn test_crypto_operations() {
    use landro_crypto::{CertificateGenerator, DeviceIdentity};
    use tempfile::tempdir;
