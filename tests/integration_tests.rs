...
mod integration_tests {
    ...
    async fn test_bloom_diff_protocol_integration() {
        ...
        assert!(result.is_ok(), "Bloom diff protocol should complete within timeout");
        
        // Test bandwidth savings
        let (total_saved, avg_saved) = diff_handler.get_bandwidth_stats().await;
        println!("📊 Bandwidth savings: {} bytes total, {:.1} average", total_saved, avg_saved);
    ...
    async fn test_enhanced_file_transfer_with_resume() {
        ...
        let bloom_engine = BloomSyncEngine::new(
        ...
        let _session_rx = resume_manager.start_resumable_session(
        ...
        let can_resume = resume_manager.can_resume_session("test_session").await.unwrap();
        assert!(can_resume, "Session should be resumable");
        
        // Test CLI progress API
        let cli_api = CliProgressApi::new(100);
        let _progress_rx = cli_api.start_session_tracking(
    ...
    async fn test_performance_benchmarks() {
        ...
        assert!(init_time < Duration::from_secs(5), "Initialization should be fast");
        ...
        for i in 0..10 {
            ...
            let _rx = cli_api.start_session_tracking(
                session_id,
                format!("peer_{}", i),
                format!("/test/path/{}", i),
            ).await;
        ...
        println!("⚡ Concurrent session creation time (10 sessions): {:?}", concurrent_time);
        
        assert!(concurrent_time < Duration::from_secs(2), "Concurrent operations should be efficient");
        assert_eq!(sessions.len(), 10, "All concurrent sessions should be created");
    ...
    async fn test_error_handling_and_recovery() {
        ...
        let can_resume = resume_manager.can_resume_session("nonexistent_session").await.unwrap();
        ...
        assert!(progress.is_none(), "Non-existent session should return None");
    ...
    async fn test_data_integrity_and_consistency() {
        ...
        let _rx = resume_manager.start_resumable_session(
        ...
        let can_resume = resume_manager.can_resume_session("integrity_test").await.unwrap();
        ...
        assert_eq!(cleanup_count, 0, "No old checkpoints should be cleaned up in fresh test");
