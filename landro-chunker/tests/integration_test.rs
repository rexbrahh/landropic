...
fn test_deduplication_effectiveness() {
    ...
    println!("Deduplication test: {} total chunks, {} unique, {:.1}% dedup ratio",
             total_chunks, unique_count, dedup_ratio * 100.0);
    
    // We should have significant deduplication since we duplicated the data
    assert!(dedup_ratio > 0.3, "Deduplication ratio {:.1}% is too low", dedup_ratio * 100.0);
}
...
fn test_incremental_changes() {
    ...
    let original_hashes: HashMap<_, _> = chunks_original.iter()
        .map(|c| (c.hash, c))
        .collect();
    ...
    println!("Incremental change test: {}/{} chunks unchanged (expected at least {})",
             identical_chunks, chunks_original.len(), expected_unchanged);
    
    // At least all but 2 chunks should be unchanged
    assert!(identical_chunks >= expected_unchanged, 
...
fn test_chunk_alignment() {
    ...
    println!("Chunk alignment test: {} + {} chunks = {} chunks combined",
             chunks1.len(), chunks2.len(), chunks_combined.len());
}
...
async fn test_async_streaming() {
    ...
    println!("Async streaming test: {} chunks match between sync and async",
             chunks_sync.len());
}
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        ...
        if i < chunks.len() - 1 {
            assert!(size >= config.min_size,
                    "Chunk {} size {} is below min_size {}", i, size, config.min_size);
        }
        
        // All chunks must be <= max_size
        assert!(size <= config.max_size,
                "Chunk {} size {} exceeds max_size {}", i, size, config.max_size);
    }
