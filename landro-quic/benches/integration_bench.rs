...
fn bench_real_file_transfer(c: &mut Criterion) {
    ...
    for (path, size) in &test_files {
        ...
        group.bench_with_input(
            ...
            |b, &(path, size)| {
                b.to_async(&rt).iter(|| async {
                    benchmark_file_transfer_lan(path.clone(), *size).await
                });
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, &(path, size)| {
                b.to_async(&rt).iter(|| async {
                    benchmark_file_transfer_wan(path.clone(), *size).await
                });
            },
        );
    }
...
fn bench_file_reading_methods(c: &mut Criterion) {
    ...
    for (path, size) in &test_files {
        ...
        group.bench_with_input(
            ...
            |b, &(path, size)| {
                b.to_async(&rt).iter(|| async {
                    let reader = ZeroCopyFileReader::new(path, 4 * 1024 * 1024).await.unwrap();
                    let mut total = 0;
                    let mut offset = 0;
        ...
        group.bench_with_input(
            ...
            |b, &chunk_size| {
                b.to_async(&rt).iter(|| async move {
                    transfer_with_chunk_size(file_size, chunk_size).await
                });
            },
        );
    }
...
fn bench_bandwidth_utilization(c: &mut Criterion) {
    ...
    group.bench_function("single_connection", |b| {
        b.to_async(&rt).iter(|| async {
            measure_bandwidth_utilization(1, file_size).await
        });
    });
    
    group.bench_function("multi_connection", |b| {
        b.to_async(&rt).iter(|| async {
            measure_bandwidth_utilization(4, file_size).await
        });
    });
...
async fn measure_bandwidth_utilization(connections: usize, total_size: usize) -> f64 {
    ...
    for _ in 0..connections {
        handles.push(tokio::spawn(async move {
            // Simulate transfer
            tokio::time::sleep(Duration::from_millis((size_per_connection / 1_000_000) as u64)).await;
            size_per_connection
        }));
    }
