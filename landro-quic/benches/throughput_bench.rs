...
use landro_quic::{
    ParallelTransferConfig, ParallelTransferManager, QuicClient, QuicConfig, QuicServer,
};
use std::io::Write;
use std::sync::Arc;
...
fn bench_throughput(c: &mut Criterion) {
    ...
    for (size, label) in sizes {
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                b.to_async(&rt)
                    .iter(|| async move { transfer_single_stream(size).await });
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                b.to_async(&rt)
                    .iter(|| async move { transfer_parallel_streams(size).await });
            },
        );

        group.bench_with_input(BenchmarkId::new("zero_copy", label), &size, |b, &size| {
            b.to_async(&rt)
                .iter(|| async move { transfer_zero_copy(size).await });
        });
    }
...
fn bench_latency(c: &mut Criterion) {
    ...
    let sizes = vec![(512, "512B"), (1024, "1KB"), (4096, "4KB"), (16384, "16KB")];

    for (size, label) in sizes {
        group.bench_with_input(BenchmarkId::new("small_file", label), &size, |b, &size| {
            b.to_async(&rt)
                .iter(|| async move { transfer_small_file(size).await });
        });
    }
...
fn bench_concurrent_connections(c: &mut Criterion) {
    ...
    for count in connection_counts {
        group.bench_with_input(
            ...
            |b, &count| {
                b.to_async(&rt)
                    .iter(|| async move { handle_concurrent_connections(count).await });
            },
        );
    }
...
fn bench_stream_multiplexing(c: &mut Criterion) {
    ...
    for count in stream_counts {
        group.bench_with_input(BenchmarkId::new("streams", count), &count, |b, &count| {
            b.to_async(&rt)
                .iter(|| async move { multiplex_streams(count).await });
        });
    }
...
fn bench_adaptive_parallelism(c: &mut Criterion) {
    ...
    group.bench_function("adaptive_enabled", |b| {
        b.to_async(&rt)
            .iter(|| async { transfer_with_adaptive_parallelism(true).await });
    });

    group.bench_function("adaptive_disabled", |b| {
        b.to_async(&rt)
            .iter(|| async { transfer_with_adaptive_parallelism(false).await });
    });
...
fn bench_congestion_control(c: &mut Criterion) {
    ...
    group.bench_function("cubic", |b| {
        b.to_async(&rt)
            .iter(|| async { transfer_with_congestion_control("cubic", file_size).await });
    });

    group.bench_function("bbr", |b| {
        b.to_async(&rt)
            .iter(|| async { transfer_with_congestion_control("bbr", file_size).await });
    });
