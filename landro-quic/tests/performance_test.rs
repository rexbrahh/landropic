...
async fn test_lan_throughput_large_files() {
    ...
    let server_config = QuicConfig::lan_optimized().bind_addr("127.0.0.1:0".parse().unwrap());
    ...
    let client = QuicClient::new(client_identity, verifier.clone(), client_config)
        .await
        .unwrap();
    ...
    let transfer_manager = ParallelTransferManager::new(Arc::new(connection), parallel_config);
...
async fn test_concurrent_connections() {
    ...
    for i in 0..target_connections {
        ...
        let handle = tokio::spawn(async move {
            let client_identity =
                Arc::new(DeviceIdentity::generate(&format!("client-{}", i)).unwrap());

            let client_config = QuicConfig::lan_optimized();
            let client = QuicClient::new(client_identity, verifier, client_config)
                .await
                .unwrap();
    ...
    println!(
        "Successfully established {} concurrent connections",
        successful
    );
...
async fn test_zero_copy_performance() {
    ...
    let start = Instant::now();
    let reader = ZeroCopyFileReader::new(temp_file.path(), 1024 * 1024)
        .await
        .unwrap();
