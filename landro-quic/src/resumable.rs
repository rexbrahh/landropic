...
impl ResumableTransferManager {
    ...
    pub async fn start_transfer(
        ...
        info!(
            "Starting resumable transfer: {} ({} bytes)",
        ...
        let bytes_transferred = checkpoint
            .as_ref()
    ...
    pub async fn update_progress(
        ...
        if let Some(transfer) = active_transfers.get_mut(transfer_id) {
            ...
            let elapsed =
                chrono::Utc::now().signed_duration_since(transfer.checkpoint.last_checkpoint);
            ...
            if elapsed.num_seconds() >= self.checkpoint_interval.as_secs() as i64 {
                ...
                debug!(
                    "Checkpointed transfer {} at {} bytes",
