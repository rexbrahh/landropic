...
use landro_proto::{
    ...
    Ack, ChunkData, Error as ProtoError, FolderSummary, Manifest, Want,
};
...
impl StreamProtocol {
    ...
    pub async fn send_folder_summary(&self, summary: FolderSummary) -> Result<()> {
        ...
        self.send_message(&mut stream, MessageType::FolderSummary, &summary)
            .await?;
        stream
            .finish()
    ...
    pub async fn receive_folder_summary(&self) -> Result<FolderSummary> {
        ...
        let summary: FolderSummary = self
            .receive_message(&mut stream, MessageType::FolderSummary)
            .await?;
    ...
    pub async fn send_manifest(&self, manifest: Manifest) -> Result<()> {
        ...
        let mut stream = self.connection.open_uni().await?;
        self.send_message(&mut stream, MessageType::Manifest, &manifest)
            .await?;
        stream
            .finish()
            .map_err(|e| QuicError::Stream(format!("Failed to finish stream: {}", e)))?;
        ...
        info!(
            "Sent manifest for folder {} with {} files",
            manifest.folder_id,
            manifest.files.len()
        );
        Ok(())
    ...
    pub async fn receive_manifest(&self) -> Result<Manifest> {
        ...
        let manifest: Manifest = self
            .receive_message(&mut stream, MessageType::Manifest)
            .await?;
        ...
        info!(
            "Received manifest for folder {} with {} files",
            manifest.folder_id,
            manifest.files.len()
        );
        Ok(manifest)
    ...
    pub async fn send_want(&self, want: Want) -> Result<()> {
        debug!(
            "Sending want request for folder {} with {} chunks",
            want.folder_id,
        ...
        let mut stream = self.connection.open_uni().await?;
        self.send_message(&mut stream, MessageType::Want, &want)
            .await?;
        stream
            .finish()
            .map_err(|e| QuicError::Stream(format!("Failed to finish stream: {}", e)))?;
    ...
    pub async fn receive_want(&self) -> Result<Want> {
        ...
        info!(
            "Received want request for {} chunks",
            want.chunk_hashes.len()
        );
        Ok(want)
    }
    ...
    pub async fn send_chunk_data(&self, chunk: ChunkData) -> Result<()> {
        ...
        let mut stream = self.connection.open_uni().await?;
        self.send_message(&mut stream, MessageType::ChunkData, &chunk)
            .await?;
        stream
            .finish()
            .map_err(|e| QuicError::Stream(format!("Failed to finish stream: {}", e)))?;
    ...
    pub async fn receive_chunk_data(&self) -> Result<ChunkData> {
        ...
        let chunk: ChunkData = self
            .receive_message(&mut stream, MessageType::ChunkData)
            .await?;
    ...
    pub async fn send_ack(&self, ack: Ack) -> Result<()> {
        ...
        let mut stream = self.connection.open_uni().await?;
        self.send_message(&mut stream, MessageType::Ack, &ack)
            .await?;
        stream
            .finish()
            .map_err(|e| QuicError::Stream(format!("Failed to finish stream: {}", e)))?;
    ...
    pub async fn send_error(&self, error: ProtoError) -> Result<()> {
        ...
        let mut stream = self.connection.open_uni().await?;
        self.send_message(&mut stream, MessageType::Error, &error)
            .await?;
        stream
            .finish()
            .map_err(|e| QuicError::Stream(format!("Failed to finish stream: {}", e)))?;
    ...
    pub async fn receive_error(&self) -> Result<ProtoError> {
        let mut stream = self.connection.accept_uni().await?;
        let error: ProtoError = self
            .receive_message(&mut stream, MessageType::Error)
            .await?;
    ...
    pub async fn receive_any_message(&self) -> Result<ProtocolMessage> {
        ...
        let message_type = MessageType::from_u8(type_byte[0]).ok_or_else(|| {
            QuicError::Protocol(format!("Unknown message type: {}", type_byte[0]))
        })?;
        ...
        let message = match message_type {
            MessageType::Hello => {
                return Err(QuicError::Protocol(
                    "Hello messages should be handled separately".to_string(),
                ));
            }
            MessageType::FolderSummary => {
                let summary = self
                    .receive_typed_message::<FolderSummary>(&mut stream)
                    .await?;
    ...
    async fn receive_typed_message<T: Message + Default>(
        &self,
        stream: &mut RecvStream,
    ) -> Result<T> {
        // Read message length
            ...
            match self.protocol.receive_chunk_data().await {
                Ok(chunk) => {
                    ...
                    if wanted_chunks.contains(&hash) {
                        ...
                        warn!(
                            "Received unexpected chunk with hash: {}",
                            hex::encode(&hash)
                        );
                    }
                }
