...
impl StreamTransferManager {
    ...
    pub fn new(connection: Arc<Connection>, config: StreamTransferConfig) -> Self {
        ...
        let priority_scheduler = Arc::new(PriorityScheduler::new(config.enable_stream_priority));
    ...
    pub async fn stream_chunks(
        ...
        info!(
            "Starting stream transfer of {} chunks with priority {}",
            chunks.len(),
            priority
        );
        ...
        for batch in scheduled_chunks.chunks(self.config.max_concurrent_streams) {
            for chunk in batch {
                ...
                let flow_permit = self
                    ...
                    .write()
                    .await
                    .acquire_capacity(chunk.data.len())
                    .await?;
                ...
                tasks.spawn(async move {
                    ...
                    if let Ok(ref transfer_result) = result {
                        bytes_transferred
                            .fetch_add(transfer_result.bytes_transferred as u64, Ordering::Relaxed);
                    }

                    active_streams.fetch_sub(1, Ordering::Relaxed);

                    let _ = result_tx
                        .send(result.unwrap_or_else(|e| TransferResult {
                            chunk_hash: vec![],
                            success: false,
                            bytes_transferred: 0,
    ...
    async fn transfer_chunk_stream(
        ...
        while attempts < config.max_retransmit_attempts {
            ...
            .await
            {
                Ok(_) => {
                    debug!(
                        "Successfully transferred chunk {} ({} bytes)",
                ...
                Err(e) => {
                    warn!(
                        "Transfer attempt {} failed for chunk {}: {}",
                        attempts,
                        hex::encode(&chunk_hash),
                        e
                    );
    ...
    async fn send_chunk_with_timeout(
        ...
        tokio::time::timeout(timeout, async {
            ...
            send.write_all(&[MessageType::ChunkData as u8])
                .await
    ...
    pub async fn stream_diff(&self, diff_request: DiffRequest) -> Result<DiffResponse> {
        debug!(
            "Processing diff request for {} chunks",
            diff_request.chunk_hashes.len()
        );
        ...
        send.write_all(&[MessageType::DiffRequest as u8])
            .await
            .map_err(|e| QuicError::Stream(format!("Failed to write header: {}", e)))?;
    ...
    pub async fn create_multiplexed_stream(
        ...
        for _ in 0..num_substreams {
            ...
            substreams
                .write()
                .await
                .push((Arc::new(Mutex::new(send)), Arc::new(Mutex::new(recv))));
        }
...
impl FlowController {
    ...
    async fn acquire_capacity(&mut self, size: usize) -> Result<FlowPermit> {
        ...
        if current < threshold {
            debug!(
                "Applying backpressure: {} / {} bytes available",
    ...
    async fn acquire(&self) -> Result<StreamHandle> {
        let _permit =
            self.semaphore.acquire().await.map_err(|e| {
                QuicError::Stream(format!("Failed to acquire stream permit: {}", e))
            })?;
