...
impl ParallelTransferManager {
    ...
    pub async fn transfer_chunks(
        ...
        for (batch_id, batch) in chunk_batches.into_iter().enumerate() {
            ...
            tasks.spawn(async move {
                ...
                let _permit = stream_sem.acquire().await.map_err(|e| {
    ...
    async fn calculate_optimal_streams(&self, total_chunks: usize) -> usize {
        ...
        let optimal = if bandwidth > 100_000_000.0 {
            // > 100 MB/s
            self.config.max_concurrent_streams
        } else if bandwidth > 50_000_000.0 {
            // > 50 MB/s
            (self.config.max_concurrent_streams * 3) / 4
        } else if bandwidth > 10_000_000.0 {
            // > 10 MB/s
        ...
        adjusted
            .max(1)
            .min(total_chunks)
            .min(self.config.max_concurrent_streams)
    }
    ...
    async fn transfer_batch(
        ...
        debug!(
            "Starting batch {} transfer with {} chunks",
            batch_id,
            chunk_hashes.len()
        );
        ...
        for hash in &chunk_hashes {
            ...
            let _mem_permit = memory_limiter
    ...
    async fn send_chunk(stream: &mut SendStream, chunk: &ChunkData) -> Result<()> {
        // Write message type
        stream
            .write_all(&[MessageType::ChunkData as u8])
            .await
...
mod tests {
    ...
    fn test_chunk_partitioning() {
        ...
        let chunks: Vec<Vec<u8>> = (0..100).map(|i| vec![i as u8]).collect();
