...
use landro_quic::{
    ...
    stream_transfer::{StreamTransferManager, StreamTransferConfig, TransferResult},
    protocol::MessageType,
};
use landro_proto::{ChunkData, Hello, FolderSummary, Manifest as ProtoManifest, Want, Ack};
use prost::Message;

use crate::{
    protocol::{SyncSession, SessionState, TransferStats},
    errors::{Result, SyncError},
    sync_persistence::{SyncPersistenceManager, PersistedSyncSession, SyncSessionStatus},
    conflict_detection::{ConflictDetector, ConflictDetectionConfig, Conflict},
};
...
impl QuicSyncTransport {
    ...
    pub async fn start_sync(
        ...
        let device_id = self.connection.remote_device_id().await
            .ok_or_else(|| SyncError::Protocol("No remote device ID".to_string()))?;
        let device_name = self.connection.remote_device_name().await
            .unwrap_or_else(|| "Unknown".to_string());
        
        // Create sync session with in-memory database
        let database = crate::state::AsyncSyncDatabase::open_in_memory().await
        ...
        self.persistence_manager.start_session(persisted_session).await?;
    ...
    async fn initiate_sync(
        ...
        self.send_message(MessageType::FolderSummary, &summary).await?;
    ...
    async fn respond_to_sync(
        ...
        let summary = self.receive_message::<FolderSummary>(MessageType::FolderSummary).await?;
        let response = session.handle_folder_summary(summary).await?;
        self.send_message(MessageType::FolderSummary, &response).await?;
    ...
    async fn handle_sync_flow(
        &self,
        session: Arc<SyncSession>,
    ) -> Result<()> {
        // Exchange manifests
        let manifest = self.receive_message::<ProtoManifest>(MessageType::Manifest).await?;
        let want = session.handle_manifest(manifest).await?;
        self.send_message(MessageType::Want, &want).await?;
        
        // Start chunk transfer
        self.transfer_chunks(session.clone(), want.chunk_hashes).await?;
    ...
    async fn transfer_chunks(
        ...
        let mut result_rx = self.transfer_manager
        ...
        tokio::spawn(async move {
            while let Some(result) = result_rx.recv().await {
                if result.success {
                    debug!("Chunk {} transferred successfully", hex::encode(&result.chunk_hash));
                    
                    // Update persistence with completed chunk
                    let chunk_hash_str = hex::encode(&result.chunk_hash);
                    if let Err(e) = persistence_manager
                        .mark_chunk_completed(&session_id_clone, chunk_hash_str, result.chunk_hash.len() as u64)
                        .await
                    {
    ...
    async fn monitor_transfer_progress(
        &self,
        session: Arc<SyncSession>,
    ) -> Result<()> {
    ...
    pub async fn resume_sync(&self, session_id: &str) -> Result<()> {
        ...
        let persisted_session = self.persistence_manager
            .get_session(session_id)
            .await
            .ok_or_else(|| SyncError::Protocol(format!("No persisted session found: {}", session_id)))?;
        ...
        self.active_sessions.write().await.insert(session_id.to_string(), session.clone());
        
        // Resume transfer of pending and failed chunks
        let mut chunks_to_transfer: Vec<_> = persisted_session.pending_chunks
    ...
    pub async fn complete_sync(&self, session_id: &str) -> Result<()> {
        ...
        self.persistence_manager.complete_session(session_id).await?;
    ...
    pub async fn interrupt_sync(&self, session_id: &str, reason: &str) -> Result<()> {
        ...
        self.persistence_manager.interrupt_session(session_id).await?;
