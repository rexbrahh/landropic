...
use landro_sync::{SyncConfig, SyncOrchestrator};
...
impl Daemon {
    ...
    pub async fn start(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        self.start_background_tasks(quic_server, sync_orchestrator)
            .await?;
    ...
    async fn start_quic_server(&self) -> Result<Arc<QuicServer>, String> {
        ...
        let identity = Arc::new(
            landro_crypto::DeviceIdentity::generate(&self.config.device_name)
    ...
    async fn start_discovery_service(
        &self,
        port: u16,
    ) -> Result<Arc<Mutex<DiscoveryService>>, String> {
        info!("Starting mDNS discovery service");

        let mut discovery =
            DiscoveryService::new(&self.config.device_name).map_err(|e| e.to_string())?;
    ...
    pub async fn add_sync_folder(
                    ...
                    if auto_sync {
                        ...
                        if let Ok(peers) = discovery.lock().await.browse_peers().await {
                            for peer in peers {
                                ...
                                if let Err(e) = sync_orch
                                    .start_sync(peer.device_id.clone(), peer.device_name.clone())
                                    .await
                                {
                                    warn!(
                                        "Failed to start sync with peer {} after file change: {}",
                                        peer.device_name, e
                                    );
                                } else {
                                    info!(
                                        "Started sync with peer {} for file change",
                                        peer.device_name
                                    );
                                }
                            }
        ...
        state
            ...
            .as_ref()
            .unwrap()
            .index_folder(&path_for_indexing)
            .await?;
