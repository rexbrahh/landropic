...
use landro_sync::{
    diff::{ChangeType, DiffComputer, FileChange},
    QuicSyncTransport, SyncError as SyncCrateError, TransferStats,
};

use crate::bloom_diff::{
    compression, DiffProgress, DiffProtocolHandler, DiffProtocolMessage, DiffStage, ManifestSummary,
};
...
impl BloomSyncEngine {
    ...
    pub async fn new(
        ...
        let quic_transport = Arc::new(DummyQuicSyncTransport { cas: cas.clone() });
    ...
    pub async fn start_enhanced_sync(
        ...
        tokio::spawn(async move {
            if let Err(e) = self_clone
                .run_enhanced_sync_protocol(&session_id_clone)
                .await
            {
    ...
    async fn run_enhanced_sync_protocol(
        ...
        info!(
            "📊 Running enhanced sync protocol for session {}",
            session_id
        );
        ...
        let diff_result = self
            .run_bloom_diff_phase(&peer_id, &folder_path, session_id)
            .await?;

        // Phase 2: Efficient File Transfer
        let transfer_result = self
            .run_transfer_phase(&peer_id, &folder_path, session_id, diff_result)
            .await?;

        // Phase 3: Verification and Completion
        self.complete_sync_session(session_id, transfer_result)
            .await?;
    ...
    async fn run_bloom_diff_phase(
        ...
        self.update_session_stage(session_id, DiffStage::ExchangingSummaries)
            .await;
        ...
        let remote_summary = self
            .simulate_remote_summary_response(&local_manifest)
            .await?;
        let diff_details = self
            .process_bloom_diff_exchange(&local_manifest, &remote_summary, session_id)
            .await?;
    ...
    async fn run_transfer_phase(
        ...
        info!(
            "🚛 Phase 2: File transfer ({} files)",
            diff_result.differences_found
        );

        self.update_session_stage(session_id, DiffStage::TransferringChunks)
            .await;
        ...
        for file_diff in &diff_result.files_to_transfer {
            info!(
                "📤 Transferring: {} ({} bytes)",
                file_diff.path, file_diff.transfer_size
            );
            ...
            for (i, chunk_hash) in file_diff.required_chunks.iter().enumerate() {
                ...
                self.update_transfer_progress(session_id, progress_percent as u8)
                    .await;
        ...
        info!(
            "✅ Transfer phase completed: {} files, {} bytes in {:?} ({} B/s)",
    ...
    async fn complete_sync_session(
        ...
        self.update_session_stage(session_id, DiffStage::Completed)
            .await;
        ...
        if let Some(session) = sessions.get_mut(session_id) {
            ...
            session.stats.bandwidth_savings.actual_bytes_transferred =
            ...
            info!(
                "📈 Final stats for {}: {} files, {} bytes, {:.2}s, {:.1}% bandwidth saved",
                session_id,
    ...
    async fn update_transfer_progress(&self, session_id: &str, progress_percent: u8) {
        debug!(
            "📊 Session {} transfer progress: {}%",
            session_id, progress_percent
        );
    ...
    async fn process_bloom_diff_exchange(
        ...
        self.update_session_stage(session_id, DiffStage::ComparingManifests)
            .await;
        ...
        info!(
            "🎯 Diff analysis: {} changes found, {} false positives, {} true negatives",
            file_changes.len(),
            false_positives,
            true_negatives
        );
...
mod tests {
    ...
    async fn test_enhanced_sync_initialization() {
        ...
        let engine = BloomSyncEngine::new("test_device".to_string(), cas, indexer)
            .await
            .unwrap();
