...
use landro_daemon::orchestrator::{
    SyncOrchestrator, OrchestratorMessage, OrchestratorConfig, OrchestratorStatus
};
...
async fn test_orchestrator_file_pipeline() {
    ...
    sender.send(OrchestratorMessage::AddSyncFolder(test_folder.clone())).await.unwrap();
    ...
    sender.send(OrchestratorMessage::FileChangesBatch(batch_events)).await.unwrap();
    ...
    sender.send(OrchestratorMessage::RemoveSyncFolder(test_folder.clone())).await.unwrap();
...
async fn test_orchestrator_peer_discovery() {
    ...
    sender.send(OrchestratorMessage::PeerDiscovered(peer)).await.unwrap();
    ...
    // Simulate peer loss
    sender.send(OrchestratorMessage::PeerLost("test-peer-001".to_string())).await.unwrap();
...
async fn test_orchestrator_concurrent_operations() {
    ...
    for i in 0..20 {
        ...
        let task = tokio::spawn(async move {
            ...
            tokio::fs::write(&file_path, format!("Concurrent content {}", i)).await.unwrap();
