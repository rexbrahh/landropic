...
async fn test_file_change_pipeline_indexing() {
    ...
    daemon
        .add_sync_folder(&sync_folder)
        .await
        .expect("Failed to add sync folder");
    ...
    fs::write(&test_file2, "second file content")
        .await
    ...
    fs::rename(&test_file2, &renamed_file)
        .await
...
async fn test_file_change_pipeline_with_auto_sync() {
    ...
    fs::write(&test_file, "auto sync content")
        .await
        .expect("Failed to create test file");
    ...
    for i in 0..5 {
        ...
        fs::write(&file_path, format!("batch content {}", i))
            .await
            .expect("Failed to create batch file");
...
async fn test_file_change_pipeline_large_files() {
    ...
    fs::write(&large_file, modified_large_content)
        .await
        .expect("Failed to modify large file");
